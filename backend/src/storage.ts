...
function tasksFromDb(db: DatabaseSync): TaskRecord[] {
  return (db.prepare('SELECT * FROM tasks ORDER BY created_at DESC, id').all() as any[]).map((row) => ({
    ...
    skipHitAndRun: fromBool(row.skip_hit_and_run),
    ...
    sortRule: row.sort_rule ?? undefined,
    fetchLimit: row.fetch_limit ?? 100,
...
function torrentFromRow(row: any): TorrentRecord {
  return {
    ...
    pushUnconfirmed: row.push_unconfirmed === null || row.push_unconfirmed === undefined ? undefined : fromBool(row.push_unconfirmed)
  }
}
...
function taskFieldToDbValue(key: string, value: unknown): unknown {
  ...
  if (key === 'autoRunEnabled' || key === 'onlyFreeDownload' || key === 'deleteOnFreeExpire' || key === 'skipHitAndRun' || key === 'autoPush' || key === 'running') {
...
function refreshTorrentFreeStateForStorage(torrent: TorrentRecord) {
...
export async function refreshStoredTorrentFreeStates() {
  ...
  for (const torrent of candidates) {
    if (refreshTorrentFreeStateForStorage(torrent)) changed.push(torrent)
  }
