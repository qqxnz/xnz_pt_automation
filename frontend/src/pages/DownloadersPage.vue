  ...
  <AppLayout>
    <section class="downloaders-page">
      <CCPageHeader
        eyebrow="下载器管理"
        title="下载器运行状态"
        description="统一查看连接健康、实时传输与默认推送配置。"
        :meta="lastDownloaderSyncText"
      >
        <template #actions>
          <button
        ...
        <main class="downloaders-main">
          <section class="cc-card cc-list-surface">
            ...
            <div class="panel-title-row">
              <h2>下载器列表</h2>
              <span>卡片汇总连接、传输与路径信息 · {{ items.length }} 个</span>
            </div>

            <CCStateView
              v-if="loading && !items.length"
              title="正在加载下载器"
              description="正在读取连接与实时传输状态。"
              tone="loading"
            />
            <CCStateView
              v-else-if="!items.length"
              title="还没有下载器"
              description="添加 qBittorrent 下载器后，任务就可以推送种子。"
            >
              <button
                class="primary-button compact"
                type="button"
              ...
              >
                <div class="downloader-card-main">
                  <div class="downloader-card-title">
                    ...
                    <span
                      class="chip"
                      :class="statusMeta(displayStatus(downloader)).className"
                      >{{ statusMeta(displayStatus(downloader)).label }}</span
                    >
                    <span
                      v-if="downloader.hasIpv6Peers"
                      class="chip ipv6-chip"
                      :title="`${downloader.ipv6TorrentCount ?? 0} 个种子有 IPV6 peer 连接`"
                      >IPv6</span
                    >
                    <span v-if="!downloader.enabled" class="chip muted-chip"
                      >已禁用</span
                    >
                  </div>
                  <dl class="downloader-card-meta">
                    ...
                    <div>
                      ...
                      <dd>
                        {{
                          formatDate(
                            statusById[downloader.id]?.lastSyncedAt ||
                              downloader.lastSyncedAt,
                          )
                        }}
                      </dd>
                    </div>
                  ...
                  <div class="downloader-card-status-grid">
                    <article>
                      ...
                      <strong class="success">{{
                        formatSpeed(statusById[downloader.id]?.uploadSpeed)
                      }}</strong>
                    </article>
                    <article>
                      <span>下载速度</span>
                      <strong>{{
                        formatSpeed(statusById[downloader.id]?.downloadSpeed)
                      }}</strong>
                    </article>
                  </div>
                  <p
                  ...
                  <section
                    v-if="expandedDownloaderId === downloader.id"
                    class="downloader-torrent-status"
                  >
                    <div class="panel-title-row">
                      <h4>种子状态</h4>
                      <span
                        >{{
                          downloaderTorrents[downloader.id]?.length ?? 0
                        }}
                        条</span
                      >
                    </div>
                    <div
                      v-if="downloaderTorrentLoading[downloader.id]"
                      class="empty-tip"
                    >
                      正在读取下载器种子...
                    </div>
                    <div
                      v-else-if="downloaderTorrentErrors[downloader.id]"
                      class="error-banner"
                    >
                      {{ downloaderTorrentErrors[downloader.id] }}
                    </div>
                    <div
                      v-else-if="downloaderTorrents[downloader.id]?.length"
                      class="downloader-torrent-list"
                    >
                      <article
                        v-for="torrent in downloaderTorrents[downloader.id]"
                        :key="torrent.hash"
                      >
                    ...
                    <div v-else class="empty-tip">当前下载器没有种子。</div>
                  </section>
                </div>
                <div
                  class="row-actions compact-actions downloader-card-actions"
                >
                  <button
                    class="row-primary-action"
                    type="button"
                    @click="testDownloaderItem(downloader)"
                  >
                    {{
                      displayStatus(downloader) === "ONLINE"
                        ? "测试连接"
                        : "重新测试"
                    }}
                  </button>
                  <AppActionMenu
                    :items="downloaderMenuItems(downloader)"
                    :label="`${downloader.name} 的更多操作`"
                    @select="handleDownloaderMenu(downloader, $event)"
                  />
                </div>
              </article>
            </div>
    ...
    >
      <form
        class="cc-form-dialog site-form downloader-form"
        @submit.prevent="saveDownloader"
      >
        ...
        <div class="form-grid compact-form-grid">
          ...
          <section>
            ...
            <label
              >服务地址<input
                ...
                required
                placeholder="http://nas:8080"
            /></label>
            <label
              >用户名<input v-model.trim="form.username" placeholder="可选"
            /></label>
            <label>
              密码
              <div class="password-input">
                ...
                <button
                  ...
                  @click="
                    downloaderPasswordVisible = !downloaderPasswordVisible
                  "
                >
          ...
          <section>
            ...
            /></label>
            <label class="inline-check"
              ><input v-model="testAfterSave" type="checkbox" />
              保存后测试连接</label
            >
            <div
              v-if="draftTestResult"
              class="empty-tip compact-tip"
              :class="{ 'success-tip': draftTestResult.success }"
            >
              {{ draftTestResult.message }}
              <span v-if="draftTestResult.version">
                · {{ draftTestResult.version }}</span
              >
            </div>
            <div class="empty-tip compact-tip">
              分类和标签由任务推送配置决定。
            </div>
          </section>
        </div>
...
const stats = ref<DownloaderStats>({
  total: 0,
  online: 0,
  authFailed: 0,
  offline: 0,
...
const expandedDownloaderId = ref<string>();
const downloaderTorrents = ref<Record<string, DownloaderTorrentItem[]>>({});
const downloaderTorrentLoading = ref<Record<string, boolean>>({});
const downloaderTorrentErrors = ref<Record<string, string>>({});
...
const passwordPlaceholder = computed(() =>
  editingDownloaderId.value && detailHasPassword.value && !form.password
    ? "已保存，留空不修改"
    : "可选",
);
...
const totalUploadSpeed = computed(() =>
  Object.values(statusById.value).reduce(
    (total, item) => total + (item?.uploadSpeed ?? 0),
    0,
  ),
);
const totalDownloadSpeed = computed(() =>
  Object.values(statusById.value).reduce(
    (total, item) => total + (item?.downloadSpeed ?? 0),
    0,
  ),
);
const lastDownloaderSyncText = computed(() => {
  const values = items.value
    .map((item) => statusById.value[item.id]?.lastSyncedAt || item.lastSyncedAt)
    .filter(Boolean) as string[];
  if (!values.length) return "尚未同步";
  return `最近同步：${formatDate(values.sort().at(-1))}`;
});
...
async function loadDownloaders() {
  ...
  try {
    ...
    statusById.value = Object.fromEntries(
      Object.entries(statusById.value).filter(([id]) => ids.has(id)),
    );
    statusErrors.value = Object.fromEntries(
      Object.entries(statusErrors.value).filter(([id]) => ids.has(id)),
    );
    startStatusPolling();
  } catch (err) {
...
function buildPayload(): DownloaderFormPayload {
  ...
  const passwordChanged = editingDownloaderId.value
    ? form.password !== originalDownloaderPassword.value
    : Boolean(password);
  return {
    name: form.name.trim(),
    type: "QBITTORRENT",
    ...
    password: editingDownloaderId.value
      ? passwordChanged
        ? password
        : undefined
      : password,
    passwordAction: editingDownloaderId.value
...
async function saveDownloader() {
  ...
  try {
    ...
    const saved = editingDownloaderId.value
      ? await updateDownloader(editingDownloaderId.value, payload)
...
function handleDownloaderMenu(downloader: DownloaderListItem, key: string) {
  if (key === "torrents") void toggleDownloaderTorrents(downloader);
  else if (key === "edit") void openEdit(downloader);
  else if (key === "delete") void removeDownloader(downloader);
}

function downloaderMenuItems(
  downloader: DownloaderListItem,
): AppActionMenuItem[] {
  return [
    {
      key: "torrents",
      label:
        expandedDownloaderId.value === downloader.id
          ? "收起种子状态"
          : "查看种子状态",
      disabled: displayStatus(downloader) !== "ONLINE",
      disabledReason:
        displayStatus(downloader) !== "ONLINE" ? "下载器当前不在线" : undefined,
    },
    { key: "edit", label: "编辑下载器" },
    { key: "delete", label: "删除下载器", tone: "danger" },
  ];
}

async function toggleDownloaderTorrents(downloader: DownloaderListItem) {
  if (expandedDownloaderId.value === downloader.id) {
    expandedDownloaderId.value = undefined;
    return;
  }
  expandedDownloaderId.value = downloader.id;
  downloaderTorrentLoading.value = {
    ...downloaderTorrentLoading.value,
    [downloader.id]: true,
  };
  downloaderTorrentErrors.value = {
    ...downloaderTorrentErrors.value,
    [downloader.id]: "",
  };
  try {
    const result = await getDownloaderTorrents(downloader.id);
    downloaderTorrents.value = {
      ...downloaderTorrents.value,
      [downloader.id]: result.items,
    };
  } catch (err) {
    downloaderTorrentErrors.value = {
      ...downloaderTorrentErrors.value,
      [downloader.id]: err instanceof Error ? err.message : "种子状态加载失败",
    };
  } finally {
    downloaderTorrentLoading.value = {
      ...downloaderTorrentLoading.value,
