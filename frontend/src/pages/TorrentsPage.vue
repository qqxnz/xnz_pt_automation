<template>
  <AppLayout>
    <section class="feature-page">
      ...
      <section class="sites-toolbar cc-toolbar cc-card torrents-toolbar">
        ...
        >
          ...
          <option
            v-for="downloader in downloaderOptions"
            :key="downloader.id"
            :value="downloader.id"
          >
            {{ downloader.name }}
      ...
      <section class="cc-card cc-list-surface">
        ...
        <CCStateView
          ...
          title="暂无种子记录"
          description="运行任务后，自动执行或手动运行产生的种子会出现在这里。"
        >
          <router-link class="primary-button compact" to="/tasks"
            >去运行任务</router-link
          >
        </CCStateView>
        <div v-else class="desktop-table torrent-table">
          <div class="torrent-row table-head">
            <span>
              ...
              /> </span
            ><span>标题 / 站点</span><span>状态</span><span>大小 / 传输</span
            ><span>下载器 / 删除条件</span><span>操作</span>
          </div>
          <div v-for="torrent in items" :key="torrent.id" class="torrent-row">
            <input v-model="selectedIds" type="checkbox" :value="torrent.id" />
            <span
              class="torrent-title-cell"
              role="button"
              tabindex="0"
              @click="showDetail(torrent)"
              @keydown.enter="showDetail(torrent)"
            >
              <strong>{{ torrent.title }}</strong>
              <small
                >{{ torrent.siteName }} · {{ formatBytes(torrent.size) }} ·
                {{ discountText(torrent.discountType) }} ·
                {{ deleteRulesText(torrent) }} · 入库
                {{ formatDate(torrent.firstSeenAt) }}</small
              >
            </span>
            <span class="torrent-status-cell">
              <span
                ><span class="chip" :class="pushClass(torrent.pushStatus)">{{
                  pushText(torrent.pushStatus)
                }}</span
                ><span
                  v-if="
                    torrent.hasIpv6Peers && torrent.pushStatus !== 'DELETED'
                  "
                  class="chip ipv6-chip"
                  :title="`${torrent.ipv6PeerCount ?? 0}/${torrent.totalPeerCount ?? 0} 个 peer 含 IPV6`"
                  >IPv6</span
                ></span
              >
              <strong
                >{{ downloadStateText(torrent) }} ·
                {{ formatProgress(torrent.downloadProgress) }}</strong
              >
              <small>{{ torrent.errorMessage || freeText(torrent) }}</small>
            </span>
            <span class="torrent-transfer-cell">
              <strong>{{ formatBytes(torrent.size) }}</strong>
              <small
                >↑ {{ formatSpeed(torrent.uploadSpeed) }} / ↓
                {{ formatSpeed(torrent.downloadSpeed) }}</small
              >
              <small
                >累计 ↑ {{ formatBytes(torrent.uploaded) }} / ↓
                {{ formatBytes(torrent.downloaded) }}</small
              >
            </span>
            <span class="torrent-downloader-cell">
              <strong>{{ torrent.downloaderName || "未分配下载器" }}</strong>
              <small>{{ deleteRulesText(torrent) }}</small>
              <small :title="downloaderSavePathText(torrent)">{{
                downloaderSavePathText(torrent)
              }}</small>
            </span>
            <span class="row-actions compact-actions">
              <button
                ...
                type="button"
                :disabled="!canPushTorrent(torrent)"
                :title="pushDisabledReason(torrent)"
                @click="pushOne(torrent)"
              >
                {{ isPushing(torrent.id) ? "推送中..." : "推送" }}
              </button>
              <AppActionMenu
                :items="torrentMenuItems(torrent)"
                :label="`${torrent.title} 的更多操作`"
                @select="handleTorrentMenu(torrent, $event)"
              />
            </span>
          </div>
        </div>
        ...
        <div class="mobile-torrent-list">
          <article
            v-for="torrent in items"
            :key="torrent.id"
            class="cc-card cc-mobile-card torrent-card"
            @click="showDetail(torrent)"
          >
            <div>
              <label class="mobile-torrent-select" @click.stop>
                <input
                  v-model="selectedIds"
                  type="checkbox"
                  :value="torrent.id"
              ...
              <span
                v-if="torrent.hasIpv6Peers && torrent.pushStatus !== 'DELETED'"
                class="chip ipv6-chip"
                :title="`${torrent.ipv6PeerCount ?? 0}/${torrent.totalPeerCount ?? 0} 个 peer 含 IPV6`"
                >IPv6</span
              >
            ...
            <p>
              {{ torrent.siteName }} · {{ formatBytes(torrent.size) }} ·
              {{ discountText(torrent.discountType) }} ·
              {{ deleteRulesText(torrent) }}
            </p>
            <p class="cc-mobile-summary">
              {{ torrent.downloaderName || "未分配下载器" }} ·
              {{ deleteRulesText(torrent) }}
            </p>
            <div class="cc-mobile-traffic">
              ...
              ><span
                >↑ {{ formatSpeed(torrent.uploadSpeed) }} / ↓
                {{ formatSpeed(torrent.downloadSpeed) }}</span
              >
            </div>
            <p v-if="torrent.errorMessage" class="cc-mobile-error">
              {{ torrent.errorMessage }}
            </p>
            <div class="row-actions compact-actions" @click.stop>
              <button
                class="row-primary-action"
                type="button"
                :disabled="!canPushTorrent(torrent)"
                :title="pushDisabledReason(torrent)"
                @click.stop="pushOne(torrent)"
              >
                {{ isPushing(torrent.id) ? "推送中..." : "推送" }}
              </button>
              <AppActionMenu
        ...
        <div
          ...
          class="torrent-batch-bar"
          role="region"
          aria-label="批量操作"
        >
          <span class="torrent-batch-count"
          ...
          <div class="torrent-batch-actions">
            <button
              ...
              type="button"
              :disabled="!selectedPushableIds.length || batchPushing"
              @click="batchPush"
            >
              {{
                batchPushing
                  ? "批量推送中..."
                  : `批量推送${selectedPushableIds.length ? ` (${selectedPushableIds.length})` : ""}`
              }}
            </button>
            <button
              class="secondary-button danger-button"
              type="button"
              :disabled="!selectedIds.length || batchDeletingRecords"
            ...
            <button
              class="secondary-button danger-button"
              type="button"
              :disabled="!selectedDeletableTaskIds.length || batchDeletingTasks"
              @click="batchDeleteTasks"
            >
              {{
                batchDeletingTasks
                  ? "删除任务中..."
                  : `删除任务${selectedDeletableTaskIds.length ? ` (${selectedDeletableTaskIds.length})` : ""}`
              }}
            </button>
            <button
              ...
              type="button"
              :disabled="
                !selectedResetableTaskIds.length || batchResettingTasks
              "
              @click="batchResetTasks"
            >
              {{
                batchResettingTasks
                  ? "重置中..."
                  : `重置任务${selectedResetableTaskIds.length ? ` (${selectedResetableTaskIds.length})` : ""}`
              }}
            </button>
          </div>
        </div>
    ...
    <div v-if="detail" class="cc-modal-backdrop modal-backdrop">
      <section class="cc-form-dialog site-form test-result-dialog">
        <div class="cc-form-head form-head">
          <div>
            <h2>种子详情</h2>
            <p>
              {{ detail.siteName }} · {{ runModeText(detail.sourceRunMode) }} ·
              {{ detail.sourceTaskName || "-" }}
            </p>
          </div>
        ...
        <div class="test-result-list">
          <article>
            <strong>标题</strong><span>{{ detail.title }}</span>
          </article>
          <article>
            <strong>大小</strong><span>{{ formatBytes(detail.size) }}</span>
          </article>
          <article>
            <strong>优惠类型</strong
            ><span>{{ discountText(detail.discountType) }}</span>
          </article>
          <article>
            <strong>做种/下载</strong
            ><span>{{ detail.seeders ?? 0 }} / {{ detail.leechers ?? 0 }}</span>
          </article>
          <article>
            <strong>入库时间</strong
          ...
          <article v-if="detail.pushedAt">
            <strong>推送时间</strong
            ><span>{{ formatDate(detail.pushedAt) }}</span>
          </article>
          <article v-if="detail.downloadStatsSyncedAt">
            <strong>统计同步</strong
            ><span>{{ formatDate(detail.downloadStatsSyncedAt) }}</span>
          </article>
          <article>
            <strong>免费状态</strong
            ><span
              >{{ freeText(detail) }} ·
              {{ stateText(detail.currentState) }}</span
            >
          </article>
          <article>
            <strong>推送状态</strong
            ><span
              >{{ pushText(detail.pushStatus) }} ·
              {{ detail.downloaderName || "-" }}</span
            >
          </article>
          <article v-if="detail.downloaderState">
            <strong>下载器状态</strong><span>{{ detail.downloaderState }}</span>
          </article>
          <article v-if="detail.torrentHash">
            <strong>种子 Hash</strong
            ><span>{{ detail.torrentHash.slice(0, 8) }}</span>
          </article>
          <article>
            <strong>任务保存位置</strong
            ><span>{{ taskSavePathText(detail) }}</span>
          </article>
          <article>
            <strong>实际保存位置</strong
            ><span>{{ downloaderSavePathText(detail) }}</span>
          </article>
          <article>
            <strong>下载进度</strong
            ><span
              >{{ formatProgress(detail.downloadProgress) }} ·
              {{ downloadStateText(detail) }}</span
            >
          </article>
          <article>
            <strong>速度</strong
            ><span
              >↑ {{ formatSpeed(detail.uploadSpeed) }} / ↓
              {{ formatSpeed(detail.downloadSpeed) }}</span
            >
          </article>
          <article>
            <strong>分享率</strong><span>{{ formatRatio(detail.ratio) }}</span>
          </article>
          <article>
            <strong>总上传/下载</strong
            ><span
              >{{ formatBytes(detail.uploaded) }} /
              {{ formatBytes(detail.downloaded) }}</span
            >
          </article>
          <article
            v-if="detail.hasIpv6Peers && detail.pushStatus !== 'DELETED'"
          >
            <strong>IPv6 peer</strong
            ><span
              >{{ detail.ipv6PeerCount ?? 0 }} /
              {{ detail.totalPeerCount ?? 0 }} 个 peer ·
              {{ formatDate(detail.peerSyncedAt) }}</span
            >
          </article>
          <article>
            <strong>仅免费下载</strong
            ><span>{{ detail.onlyFreeDownload ? "已开启" : "已关闭" }}</span>
          </article>
          <article>
            <strong>免费到期删除</strong
            ><span>{{ detail.deleteOnFreeExpire ? "已开启" : "已关闭" }}</span>
          </article>
          <article v-if="detail.lowUploadKbps && detail.lowUploadMinutes">
            <strong>低速删除</strong
            ><span
              >&lt; {{ detail.lowUploadKbps }} KB/秒 持续
              {{ detail.lowUploadMinutes }} 分钟{{
                detail.lowUploadSince
                  ? `（${formatDate(detail.lowUploadSince)} 起）`
                  : ""
              }}</span
            >
          </article>
          <article>
            <strong>链接状态</strong
            ><span>{{ linkText(detail.linkStatus) }}</span>
          </article>
          <article v-if="detail.errorMessage">
            <strong>失败原因</strong><span>{{ detail.errorMessage }}</span>
          </article>
        </div>
        <div class="cc-form-foot form-foot detail-foot">
          <div class="form-foot-actions detail-foot-actions">
            <button
              type="button"
              class="secondary-button"
              @click="handleDetailEditDeleteCondition"
            >
              修改删除条件
            </button>
            <button
              type="button"
              class="secondary-button"
              @click="handleDetailEditDownloader"
            >
              修改下载器
            </button>
            <button
              ...
              :disabled="!canPushTorrent(detail)"
              @click="handleDetailPush"
            >
              {{ isPushing(detail.id) ? "推送中..." : "推送" }}
            </button>
            <button
              v-if="canDeleteFromDownloader(detail)"
              class="secondary-button danger-text"
              type="button"
              :disabled="isTorrentBusy(detail.id)"
              @click="handleDetailDeleteFromDownloader"
            >
              {{ isDeleting(detail.id) ? "删除中..." : "删除任务" }}
            </button>
            <button
              type="button"
              class="secondary-button"
              @click="detail = undefined"
            >
              关闭
            </button>
          </div>
    ...
    <div
      v-if="deleteConditionForm"
      class="cc-modal-backdrop modal-backdrop"
      @click.self="closeDeleteConditionForm"
    >
      <form
        class="cc-form-dialog site-form"
        @submit.prevent="submitDeleteConditionForm"
      >
        <div class="cc-form-head form-head">
          <div>
            <h2>修改删除条件</h2>
            <p>
              {{ deleteConditionForm.torrent.siteName }} ·
              {{ deleteConditionForm.torrent.title }}
            </p>
          </div>
          <button
            type="button"
            :disabled="deleteConditionForm.submitting"
            @click="closeDeleteConditionForm"
          >
            ×
          </button>
        </div>
          ...
          <section class="rule-section">
            ...
            <fieldset class="rule-group">
              ...
              <label class="inline-check"
                ><input
                  v-model="deleteConditionForm.onlyFreeDownload"
                  :disabled="deleteConditionForm.submitting"
                  type="checkbox"
                />
              ...
              <label class="inline-check"
                ><input
                  v-model="deleteConditionForm.deleteOnFreeExpire"
                  :disabled="deleteConditionForm.submitting"
                  type="checkbox"
                />
                免费到期（无视下载进度）</label
              ...
              <div class="inline-row">
                <span>上传速度低于</span>
                <input
                  v-model.number="deleteConditionForm.lowUploadKbps"
                  :disabled="deleteConditionForm.submitting"
                  type="number"
                  min="0"
                  step="1"
                ...
                <span>KB/秒，持续</span>
                <input
                  v-model.number="deleteConditionForm.lowUploadMinutes"
                  :disabled="deleteConditionForm.submitting"
                  type="number"
                  min="0"
                  step="1"
            ...
            <p v-if="deleteConditionForm.lowUploadError" class="inline-hint">
              {{ deleteConditionForm.lowUploadError }}
            </p>
          </section>
        </div>
        <div class="cc-form-foot form-foot">
          <p class="form-foot-tip">
            修改后立即生效，已推送的种子会按新条件自动判断是否删除。
          </p>
          <div class="form-foot-actions">
            ...
            <button
              ...
              type="submit"
            >
              {{ deleteConditionForm.submitting ? "保存中..." : "保存" }}
            </button>
          </div>
        </div>
      </form>
    ...
    <div
      v-if="downloaderForm"
      class="cc-modal-backdrop modal-backdrop"
      @click.self="closeDownloaderForm"
    >
      <form
        class="cc-form-dialog site-form"
        @submit.prevent="submitDownloaderForm"
      >
        <div class="cc-form-head form-head">
          <div>
            <h2>修改下载器</h2>
            <p>
              {{ downloaderForm.torrent.siteName }} ·
              {{ downloaderForm.torrent.title }}
            </p>
          </div>
          <button
            type="button"
            :disabled="downloaderForm.submitting"
            @click="closeDownloaderForm"
          >
            ×
          </button>
        </div>
            ...
            <section :class="{ 'is-locked': downloaderForm.locked }">
              ...
              <label>
                ...
                <AppSelect
                  ...
                  :placeholder="
                    enabledDownloaderOptions.length
                      ? '请选择下载器'
                      : '暂无可用下载器'
                  "
                  :options="enabledDownloaderSelectOptions"
                  :disabled="
                    downloaderForm.submitting ||
                    downloaderForm.locked ||
                    !enabledDownloaderOptions.length
                  "
                />
              </label>
              <p
                v-if="
                  !downloaderForm.locked && !enabledDownloaderOptions.length
                "
                class="inline-hint"
              >
                没有启用的下载器，请先在【下载器】中启用至少一个。
              </p>
            </section>
            <section :class="{ 'is-locked': downloaderForm.locked }">
              <h3>保存位置</h3>
              <label>
                任务保存位置
                <input
                  v-model.trim="downloaderForm.taskSavePath"
                  :disabled="downloaderForm.submitting || downloaderForm.locked"
                  placeholder="留空则使用下载器默认保存位置"
                />
              </label>
              <p v-if="!downloaderForm.locked" class="inline-hint">
                仅写入该条种子记录的【任务保存位置】字段，不会修改来源任务或下载器配置。保存后点击【推送】才会真正推送到下载器。
              </p>
            </section>
          </div>
        </div>
        <div class="cc-form-foot form-foot">
          <p class="form-foot-tip">
            本次只保存设置，不会推送。如需推送到下载器，请点击【推送】按钮。
          </p>
          <div class="form-foot-actions">
            <button
              type="button"
            ...
            <button
              ...
              :disabled="
                downloaderForm.submitting ||
                (!downloaderForm.locked && !downloaderForm.downloaderId)
              "
              type="submit"
            >
              {{ downloaderForm.submitting ? "保存中..." : "保存" }}
            </button>
          </div>
        </div>
      </form>
...
const totalPages = computed(() =>
  Math.max(Math.ceil(total.value / filters.pageSize), 1),
);
const statCards = computed(() => [
  ...
  {
    label: "总上传",
    value: formatBytes(stats.value.totalUploaded),
    className: "success",
  },
  {
    label: "总下载",
    value: formatBytes(stats.value.totalDownloaded),
    className: "",
  },
]);
...
const selectedTorrents = computed(() =>
  selectedIds.value
    ...
    .filter((item): item is TorrentItem => Boolean(item)),
);
const selectedPushableIds = computed(() =>
  selectedTorrents.value.filter(canPushTorrent).map((torrent) => torrent.id),
);
const selectedDeletableTaskIds = computed(() =>
  selectedTorrents.value
    .filter(canDeleteFromDownloader)
...
const isCurrentPageAllSelected = computed(
  () =>
    ...
    items.value.every((item) => selectedIds.value.includes(item.id)),
);
const isCurrentPagePartiallySelected = computed(
  () =>
    !isCurrentPageAllSelected.value &&
    items.value.some((item) => selectedIds.value.includes(item.id)),
);
const enabledDownloaderOptions = computed(() =>
  downloaderOptions.value.filter((downloader) => downloader.enabled),
);
...
const downloaderFilterOptions = computed(() => [
  ...
  ...downloaderOptions.value.map((downloader) => ({
  ...
  enabledDownloaderOptions.value.map((downloader) => ({
    label:
      downloader.status !== "ONLINE"
        ? `${downloader.name}（${downloaderStatusText(downloader.status)}）`
        : downloader.name,
    value: downloader.id,
  })),
);
...
async function submitDeleteConditionForm() {
  ...
  try {
    ...
    if (
      deleteConditionForm.value &&
      deleteConditionForm.value.torrent.id === form.torrent.id
    ) {
...
async function submitDownloaderForm() {
  ...
  try {
    ...
    if (
      downloaderForm.value &&
      downloaderForm.value.torrent.id === form.torrent.id
    ) {
...
async function batchPush() {
  ...
  try {
    ...
    Snackbar[result.failedCount ? "warning" : "success"](
...
async function batchDeleteRecords() {
  ...
  if (
    !window.confirm(
      `确认删除选中的 ${ids.length} 条种子记录？此操作不会删除下载器任务或文件。`,
    )
  )
    return;
  ...
  try {
    ...
    Snackbar[result.missingIds.length ? "warning" : "success"](
...
async function batchDeleteTasks() {
  ...
  if (
    !window.confirm(
      `确认删除选中的 ${ids.length} 个下载器任务并同时删除已下载文件？`,
    )
  )
    return;
...
async function batchResetTasks() {
  ...
  if (
    !window.confirm(
      `确认重置选中的 ${ids.length} 个种子的本地任务状态？此操作不会联系下载器，可用于下载器不可达时重新推送。`,
    )
  )
    return;
...
async function deleteFromDownloader(torrent: TorrentItem) {
  ...
  if (
    !window.confirm(
      `确认删除下载器任务「${torrent.title}」并同时删除已下载文件？`,
    )
  )
    return;
...
function deleteRulesText(
  ...
  if (torrent.lowUploadKbps && torrent.lowUploadMinutes)
    parts.push(`低速${torrent.lowUploadKbps}KB·${torrent.lowUploadMinutes}分`);
...
function pushDisabledReason(torrent: TorrentItem) {
  if (isPushing(torrent.id)) return "正在推送";
  if (torrent.pushStatus === "PUSHED") return "种子已推送";
  if (torrent.pushStatus === "DELETED") return "记录已删除";
  if (!torrent.downloaderId) return "请先设置下载器";
  return "";
}

function torrentMenuItems(torrent: TorrentItem): AppActionMenuItem[] {
  const busy = isTorrentBusy(torrent.id);
  return [
    { key: "detail", label: "查看详情" },
    { key: "delete-condition", label: "修改删除条件", disabled: busy },
    { key: "downloader", label: "修改下载器", disabled: busy },
    {
      key: "delete-task",
      label: isDeleting(torrent.id) ? "删除中..." : "删除下载器任务",
      tone: "danger",
      hidden: !canDeleteFromDownloader(torrent),
      disabled: busy,
    },
    {
      key: "delete-record",
...
function handleTorrentMenu(torrent: TorrentItem, key: string) {
  if (key === "detail") showDetail(torrent);
  else if (key === "delete-condition") openDeleteConditionForm(torrent);
  else if (key === "downloader") openDownloaderForm(torrent);
  else if (key === "delete-task") void deleteFromDownloader(torrent);
  else if (key === "delete-record") void deleteTorrentRecord(torrent);
}

async function deleteTorrentRecord(torrent: TorrentItem) {
  if (
    !window.confirm(
      `确认删除种子记录「${torrent.title}」？此操作不会删除下载器中的任务。`,
    )
  )
    return;
  try {
    await deleteTorrentRecords([torrent.id]);
    Snackbar.success("种子记录已删除");
    await loadTorrents();
  } catch (err) {
    Snackbar.error(err instanceof Error ? err.message : "删除记录失败");
  }
...
function freeText(torrent: TorrentItem) {
  ...
  if (!torrent.freeEndAt)
    return torrent.isFreeNow ? "免费中，未获取到过期时间" : "非免费";
