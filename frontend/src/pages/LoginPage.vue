<template>
  ...
  >
    <form class="auth-card" novalidate @submit.prevent="handleSubmit">
      ...
      <h2>登录</h2>
      <p>使用管理员账号登录 PT 自动化管理平台。</p>
      <label class="auth-field">
        <span>用户名</span>
        <input
          v-model.trim="form.username"
          :aria-invalid="attempted && !form.username"
          :disabled="submitting"
          autocomplete="username"
        />
        <small v-if="attempted && !form.username" class="auth-field-error"
          >请输入用户名</small
        >
      </label>
      <label class="auth-field">
        ...
        <div class="password-input">
          <input
            ...
            :aria-invalid="attempted && !form.password"
            :disabled="submitting"
        ...
        <small v-if="attempted && !form.password" class="auth-field-error"
          >请输入密码</small
        >
      </label>
      <button
      ...
      <div class="auth-card-note">
        <span>默认管理员账号</span><strong>admin</strong>
      </div>
    </form>
  </AuthShell>
</template>
