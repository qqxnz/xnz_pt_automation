...
# 安装
appcenter-cli install-fpk /path/to/qqxnz.pta-0.6.28.fpk
# 安装后关闭
...
- 数据目录从 0.6.24 起映射到 fnOS 共享目录 `pta/data`（在 fnOS「文件管理 → 共享文件夹」中可见）
- 卸载**不会清空**共享目录内的应用数据，数据库、备份和缓存文件将保留
...
`upgrade_init` 会检测旧版本残留数据 `$TRIM_PKGVAR/data`，并自动复制到新共享目录 `pta/data`，复制成功后在共享目录写入 `.migrated-from-legacy` 标记以避免重复处理。老数据保留在原位置，可在新版本运行正常后手动清理。

### 从 Docker compose 部署迁移到飞牛

旧版部署在 `./data/`，飞牛封装使用共享目录 `pta/data`，迁移方法：
