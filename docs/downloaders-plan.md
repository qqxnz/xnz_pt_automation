...
| 默认分类 | 文本输入 | 空 | 已保存分类 | 可选，1-64 字，不允许换行。 |
| 默认标签 | 标签输入/逗号分隔输入 | 空 | 已保存标签 | 可选，逗号或回车分隔，最多 10 个，每个 1-32 字，去重并去除空格。 |
...
- 可编辑下载器名称、服务地址、用户名、默认保存路径、分类、标签、暂停和启用状态。
- 编辑时密码不明文回显，留空不会修改旧密码。
- 编辑时可更新密码，也可二次确认后清空密码。
- 表单能拦截空名称、非法服务地址、重复名称、标签超长等错误。
- 测试连接能展示成功结果和版本信息。
